...
test.describe('Home Page', () => {
  ...
  test('should have links to portfolios and report pages', async ({ page }) => {
    await page.goto('/');
    ...
    const reportLink = page.getByRole('link', { name: 'View Lumibot + Alpaca Report' });
  ...
  test('should render the report page content', async ({ page }) => {
    await page.goto('/report');

    await expect(page.getByRole('heading', { name: 'Lumibot + Alpaca Integration Report' })).toBeVisible();
    await expect(page.getByRole('heading', { name: 'Proposed Strategies' })).toBeVisible();
    await expect(page.getByText('Opening Range Breakout (ORB)')).toBeVisible();
